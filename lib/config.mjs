...
if (ODOO_ENV === 'prod' && !config.odooUrlProd) {
  throw new Error('config.json missing odooUrlProd — required for --env prod');
}
...
// The gha door is UAT-only: there is no production equivalent, and production is
// unreachable from a cloud runner anyway (Cloudflare blocks datacenter IPs).
const onRunner = process.env.GITHUB_ACTIONS === 'true';
const baseUrl  = ODOO_ENV === 'prod' ? config.odooUrlProd : config.odooUrl;
export const ODOO_URL         = onRunner && ODOO_ENV === 'uat' && config.odooUrlGha
  ? config.odooUrlGha
  : baseUrl;

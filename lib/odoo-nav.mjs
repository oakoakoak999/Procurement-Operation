...
import { ODOO_ENV } from './config.mjs';
...
// ─── SELECT DATABASE ──────────────────────────────────────────────────────────
// UAT puts a database selector in front of everything, and the database is named
// princ-smarterp-prod-base-* even there. Production serves a single database and
// has no selector page at all, so asking for one would block until the timeout
// and fail before anything loads. Go straight to /web there; Odoo bounces an
// unauthenticated request to /web/login, which login() below already handles.
//
// env is a parameter rather than a direct ODOO_ENV read so a caller can override
// it, but it defaults to the resolved environment — no caller has to care.
export async function selectDatabase(page, odooUrl, env = ODOO_ENV) {
  if (env !== 'uat') {
    log('Production — no database selector, going straight to /web');
    await page.goto(`${odooUrl}/web`);
  ...
  // Click the most recent princ-smarterp-prod-base-* database. The explicit
  // wait is what the PO-daily copy of this used across the backfill; networkidle
  // alone says the network went quiet, not that this link exists yet.

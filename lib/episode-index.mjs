 ...
 * ({ status, printed, split, uploaded, replaced, skipped }). `baseDir` is the repo root.
...
export function appendPoDailyEpisodeRow(results, meta, baseDir) {
  ...
  const replaced = results.reduce((n, r) => n + (r.replaced || 0), 0);
  ...
  // Replacements are rare and always worth noticing, so they earn a slot only
  // when they happened — a "0 replaced" on every row would just be noise.
  const stats   = `${results.length} BU · ${active} with POs · ${printed} printed · ${uploaded} uploaded` +
                  (replaced > 0 ? ` · ${replaced} replaced` : '') + ` · ${skipped} dup-skip`;
  const notable = failedN > 0 ? `${failedN} BU failed`
                : replaced > 0 ? `${replaced} PO(s) re-issued and replaced`
                : noPO > 0 ? `${noPO} BU had no POs` : 'print + upload';

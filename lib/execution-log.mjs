...
function blockFor(r, mode) {
  ...
  // Live: the PRs that actually became POs. Validate: the PRs that passed
  // validation (no PO fired). generateMatched is already flattened to strings.
  const passList = live ? (r.generateMatched || []) : (r.passingPRNumbers || []);
  const arrow = live ? 'generated' : 'pass';

...
export async function navigateToRFQList(page) {
  ...
  // Post-generate we're already in the Purchase app, so the Home-menu + Purchase
  // clicks are redundant — but clicking them is harmless (Purchase is always in
  // the 9-dot menu), and it also makes this work from a fresh login. Keep it
  // simple and unconditional rather than branching on where we started.
  await page.getByTitle('Home Menu').click();
  await page.waitForTimeout(1200);
  await page.getByRole('menuitem', { name: 'Purchase', exact: true }).click();
  await page.waitForTimeout(3000);

 ...
 *   - IDLE <PR> - <reason>
 * Run statuses: SUCCESS | LEFTOVER | NONE | ERROR | DRY-RUN (--test rehearsal). All times Asia/Bangkok (+07).
 ...
 * happened. appendConfirmLog below follows this for both modes; the batch PR2PO
 * path (run-batch.mjs) now logs --test runs as DRY-RUN too. NOTE: the direct
 * single-BU path (odoo_pr_to_po.mjs) still skips --test — a remaining deviation.
 */
...
// { pr, reason } pairs. The PR number is the text before the first ": ".
// Exported so the leftover-table writer reads rejections identically.
export function parseRejections(rejectionReasons) {
...
function idleLines(rejectionReasons) {
  return parseRejections(rejectionReasons).map(({ pr, reason }) => `- IDLE ${pr} - ${reason}`);
}
...
function blockFor(r, mode) {
  ...
  // Test (dry-run) forces DRY-RUN so a rehearsal never shares a header token with a
  // real run (mirrors appendConfirmLog); the pass/idle lines + error line below keep
  // the detail. Live/validate precedence: ERROR > LEFTOVER > NONE > SUCCESS. A benign
  // early-exit (no pending PRs / all duplicates) sets r.status='WARN' AND r.error, but
  // that is NOT a failure — only r.status==='FAILED' or a generateError is a real ERROR.
  // The WARN case falls through to NONE (ran clean, nothing to do). Do NOT gate ERROR on
  // r.error: it's set on both benign WARNs and real FAILEDs, and would mislabel every idle BU.
  const status =
    isTest ? 'DRY-RUN'
  ...
  // Surface a real failure even under the DRY-RUN header (a crashed test still matters);
  // a benign WARN (no pending PRs) carries r.error but is not a failure, so exclude it.
  if (status === 'ERROR' || (isTest && (r.status === 'FAILED' || r.generateError))) {
 ...
 *   - FAIL <PO> - <reason>
 *
 * `mode` is 'live' (--confirm) or 'dry-run'. Both are logged. Header status is
 * DRY-RUN for any non-live run so a rehearsal never shares a token with a real
 * confirmation; live runs use SUCCESS | NONE | ERROR. The PLAN/FAIL/error lines
 * still carry the detail either way.
 */
export function appendConfirmLog(r, baseDir) {
  // A dry-run confirms nothing, so it must never read as a live SUCCESS/ERROR in
  // the header — flag it DRY-RUN (Oak, 2026-07-17: every execution logs, and a
  // test is flagged as such). The per-PO PLAN/FAIL and error lines below keep
  // the detail, so a dry-run that hit a form-read failure is still visible.
  const status =
    r.mode !== 'live' ? 'DRY-RUN'
...
/**
 * PO-Daily's own block grammar — a daily run prints the day's POs, splits them
 * per-vendor and uploads to Drive; it has no PRs, no leftover state and no
 * dry-run mode (upload de-dups instead). Shares the daily file so one day's
 * procurement activity reads in one place. `results` are po-daily runStats
 * ({ runId, bu, status, printed, split, uploaded, replaced, skipped, warnings, error }).
 *
 *   ## HH:MM +07 - PO-DAILY <BU> - <STATUS>
 * Header status: ERROR (crash) | NONE (ran clean, no POs for the day) | SUCCESS.
 * NONE is not a failure — a BU can simply have no POs dated today.
 */
export function appendPoDailyLog(results, baseDir) {
  const blocks = results.map(r => {
    const status =
      ...
      : 'SUCCESS';
    const lines = [
      `## ${timeFromRunId(r.runId)} +07 - PO-DAILY ${r.bu || '?'} - ${status}`,
      `- run_id: ${r.runId || '(none - run died early)'}`,
      `- counts: printed=${r.printed ?? 0} split=${r.split ?? 0} uploaded=${r.uploaded ?? 0} replaced=${r.replaced ?? 0} skipped_dup=${r.skipped ?? 0}`,
    ];
    if (r.warnings) lines.push(`- warnings: ${r.warnings}`);
    if (status === 'ERROR') lines.push(`- error: ${r.error || r.stoppedAt || 'unknown failure'}`);
    return lines.join('\n');
  });
  return appendBlocks(blocks.join('\n\n'), baseDir);
}

...
// The BU roster is whatever config.json lists — there is no code-level exclusion
// list. KBKJ and PNKG are excluded by being absent from config.json (Oak,
// 2026-08-04, replacing the previous hardcoded BLOCKED_BUS set). Note that
// config.json is git-ignored and per-machine, so that exclusion is per-machine
// too: it has to hold separately on each PC and in the CONFIG_JSON secret.
const buOdooPrefix   = config.buOdooPrefix;
const buLogSheets    = config.buLogSheets;
const buOrderFolders = config.buOrderFolders;
...
export function odooCredentials() {
  ...
  // Trimmed for the same reason as the CF token: these arrive from the same
  // paste-into-GitHub route, and a trailing newline in a password fails login
  // as a plain wrong-credentials error with nothing pointing at whitespace.
  const username = process.env[`ODOO_USERNAME${suffix}`]?.trim();
  const password = process.env[`ODOO_PASSWORD${suffix}`]?.trim();
